...
public class SignUpTest {
    ...
    @Test
    public void creatingNewUserAccountReturns201() throws IOException {
        String newUserPayload = readResourceIntoString("/users/user.json");
        WebResource accountsEndpoint = createEndpointClient("/accounts");
        ClientResponse response = accountsEndpoint.type(MediaType.APPLICATION_JSON)
                .post(ClientResponse.class, newUserPayload);
        assertThat(response.getStatus(), equalTo(ClientResponse.Status.CREATED.getStatusCode()));
    }
